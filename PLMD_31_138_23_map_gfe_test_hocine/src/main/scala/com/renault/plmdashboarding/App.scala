...
println("*************************")
			println("*************************")
			println("*************************")
			//println("Nbre de ligne après suppression des duplicates : "+inputReduceDF.count())
...
//join_LOGIQUE_APLT_table_gfe.show(3)

// just to test the needed number of line
//println("list number after new join "+join_LOGIQUE_APLT_table_gfe.count)
...
val table_oci_ind_front_reduce_rdd=table_oci_ind_front.select(	
  ...
  "ociInd"
   ...
   .map(f=>((
	 ...
	 import sqlContext.implicits._ 
   //1: hdfs  location for table plmd_oci_ind  
   table_oci_ind_front_reduce_post
      .write.mode(SaveMode.Overwrite)
      .orc("hdfs://bigopeclu/data/refinery/irn_67133_d4b/work/oci_ind/")
      
      
	
      //2: hdfs  location for table plmd_oci_ind_histo 
	 table_oci_ind_front_reduce_post
      .write.mode(SaveMode.Append)
      .orc("hdfs://bigopeclu/data/refinery/irn_67133_d4b/work/oci_ind_histo/")
	
	
	// creation of the hive table plmd_oci_ind 
    ...
    val fieldsAndTypes = "id STRING,reference STRING,extReference STRING,refinstEIdPhysiqueAplt STRING, enDesignation STRING,frDesignation STRING,instance STRING,PG STRING,"+
      ...
      "maturity STRING,gel STRING,lastUpdate STRING,Ref_V_CustoDiscipline STRING,RefInst_E_ParentType STRING,GFE STRING,Oci_Ind STRING,Processing_Times STRING"
	...
	// creation of the hive table plmd_oci_ind_histo 
    //path of location hive table in the datalake
    ...
    //val skipHeader  = " TBLPROPERTIES ('skip.header.line.count'='1')"
    val path2        = "'hdfs://bigopeclu/data/refinery/irn_67133_d4b/work/oci_ind_histo/'"

    val fieldsAndTypes2 = "id STRING,reference STRING,extReference STRING,refinstEIdPhysiqueAplt STRING, enDesignation STRING,frDesignation STRING,instance STRING,PG STRING,"+
      "milestone STRING,inheritedConfiguration STRING,localConfiguration STRING,architectureZone STRING,version STRING,partStatus STRING,"+
      "maturity STRING,gel STRING,lastUpdate STRING,Ref_V_CustoDiscipline STRING,RefInst_E_ParentType STRING,GFE STRING,Oci_Ind STRING,Processing_Times STRING"
      
    val request2 : String = "CREATE EXTERNAL TABLE IF NOT EXISTS db_work_irn_67133_d4b.plmd_oci_ind_histo (" + fieldsAndTypes2 + ")" +
      """  
                               STORED AS ORC  
                             LOCATION """ + path2 //+ skipHeader
          hiveContext.sql(request2)
...
table_oci_ind_front_reduce_post.write.format("org.elasticsearch.spark.sql")
      .mode(SaveMode.Overwrite)
      //.save("d4b_dev_oci_ind_template_test/1") 
      .save("d4b_ope_oci_ind/1")
